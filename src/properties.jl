...
ptd(p::S,t::T) where {S<:Real,T<:Real} =1/ptv(p,t)
...
pqt(p::S,q::T) where {S<:Real,T<:Real} =CoolProp.PropsSI("T","P",p*1.e5,"Q",q,"Water")-273.15
...
sat_dpdt(t::T) where T<:Real=CoolProp.PropsSI("d(P)/d(T)|sigma","P",t+273.15,"Q",1,"Water")/1.e5
sat_dtdp(p::T) where T<:Real=CoolProp.PropsSI("d(T)/d(P)|sigma","P",p*1e5,"Q",1,"Water")*1.e5
sat_dhdp(p::S,q::T) where {S<:Real,T<:Real} =CoolProp.PropsSI("d(Hmass)/d(P)|sigma","P",p*1e5,"Q",0,"Water")*100.
...
export pt,pth,pts,ptv,pht,phs,phv,phq,psh,pst,psv,pqh,pqs,tp,thp,tsp,tqp,ths,dpdh,dhdp,dhdt,sat_dpdt,sat_dtdp,sat_dhdp,dtdp,dtdh